...
function DatasetPage() {
    ...
    const [
        ...
        set_together_patient_gene_information,
    ] = useState([
        {
            patient_id: "",
            age: 0,
    ...
    const [gene_with_value_information, set_gene_with_value_information] =
        useState([
            {
                id: 1,
                name: "",
    ...
    const [
        ...
        set_modified_patients_list_to_update_back,
    ] = useState({});
    const [gene_information_expanded, setGene_information_expanded] = useState([
        { id: 0, gene_id: "ENT" },
    ]);
    const [gene_list_filtered, set_gene_list_filtered] = useState([
        { id: 0, gene_id: "ENT" },
    ]);
    ...
    useEffect(() => {
        const setTogetherData = async () => {
            let combined_patients_gene_data =
                get_combined_patients_genes_data();
            await set_together_patient_gene_information(
                combined_patients_gene_data
            );
    ...
    const get_combined_patients_genes_data = () => {
        ...
        for (let i = 0; i < patient_information.length; i++) {
            ...
            for (let j = 0; j < gene_with_value_information.length; j++) {
                let patient_index = gene_with_value_information[j][
                    "patient_ids"
                ]["arr"].indexOf(existing_patient_info["patient_id"]);

                gene_patient_subset_values[
                    gene_with_value_information[j]["name"]
                ] = parseFloat(
                    gene_with_value_information[j]["gene_values"]["arr"][
                        patient_index
                    ]
                );
            }
            combined_dataset_full_information.push({
            ...
            <div id="column_gene_search_input">
                <div class="input-group">
                    ...
                    <button
                        ...
                        class="btn btn-primary gene_search_btn"
                    >
    ...
    const hasLevenshteinDistanceLessThanEqualOne = (
        current_input_str,
        str_reference
    ) => {
    ...
    const filterFuzzyText = (filterVals, data) => {
        ...
        if (removed_decimal_input.includes(".")) {
            removed_decimal_input = removed_decimal_input.substring(
                0,
                removed_decimal_input.indexOf(".")
            );
        }

        // equals filter
        let filtered_list_genes = data
            .filter(
                (patient_one) =>
                    hasLevenshteinDistanceLessThanEqualOne(
                        patient_one[colName],
                        input_str
                    ) <= 1 ||
                    patient_one[colName].includes(input_str) ||
                    patient_one[colName].includes(removed_decimal_input)
            )
            .sort((patient_object_a, patient_object_b) => {
                return (
                    hasLevenshteinDistanceLessThanEqualOne(
                        patient_object_a[colName],
                        input_str
                    ) -
                    hasLevenshteinDistanceLessThanEqualOne(
                        patient_object_b[colName],
                        input_str
                    )
                );
            });
    ...
    const geneListFilter = (gene_list_filter_value) => {
        ...
        let text_search =
            gene_list_filter_value.gene_id.filterVal.input_string_value;
        ...
        let search_results_genes = filterFuzzyText(
            {
                input_string_value: text_search,
                colName: column_search,
                reset: false,
            },
            gene_information_expanded
        );
    ...
    const generateGeneTable = (gene_objs_information) => {
        ...
        if (
            gene_objs_information == null ||
            gene_objs_information.length == 0
        ) {
            return [
                {
                    dataField: "id",
                    ...
                    filterRenderer: (onFilter, column) => {
                        return (
                            <ProductFilter
                                onFilter={onFilter}
                                column={column}
                                optionsInput={clone(select_options_col)}
                            />
                        );
                    },
                };
    ...
    const generateDatasetMatrixTable = () => {
        ...
        let column_possibilities = Object.keys(
            together_patient_gene_information[0]
        );

        for (let i = 0; i < column_possibilities.length; i++) {
            let unique = [
                ...new Set(
                    together_patient_gene_information.flatMap(
                        (item) => item[column_possibilities[i]]
                    )
                ),
            ];
            ...
            if (together_patient_gene_information.length > 0) {
                ...
                if (
                    (typeof example_val === "string" ||
                        example_val instanceof String ||
                        (typeof example_val == "number" &&
                            !isNaN(example_val))) &&
                    column_possibilities[i] != "dataset_id"
                ) {
        ...
        for (let i = 0; i < filter_columns.length; i++) {
            ...
            if (current_filter.filterType == "NUMBER") {
                ...
                if (current_filter.filterVal.compareValCode == 1) {
                    // <
                    isFiltered = true;
                    matrix_filtered = matrix_filtered.filter(
                        (patient_one) =>
    ...
    const updateCellEditMatrix = async (stateChangeInfo) => {
        ...
        let patient_edited_index = copy_matrix_filtered.findIndex(
            (element) =>
                element["patient_id"] == stateChangeInfo["cellEdit"]["rowId"]
        );
        ...
        let sample_val =
            together_patient_gene_information[0][
                stateChangeInfo["cellEdit"]["dataField"]
            ];
        let type_str = "str";
        ...
        if (
            !(
                stateChangeInfo["cellEdit"]["rowId"] in
                copy_modified_patients_list
            )
        ) {
            ...
            if (type_str == "int") {
                new_patient_update[data_field_key] = parseInt(
                    String(stateChangeInfo["cellEdit"]["newValue"])
                );
            } else if (type_str == "float") {
                new_patient_update[data_field_key] = parseFloat(
                    String(stateChangeInfo["cellEdit"]["newValue"])
                );
            } else {
                new_patient_update[data_field_key] = String(
                    stateChangeInfo["cellEdit"]["newValue"]
                ).toLowerCase();
            }

            copy_modified_patients_list[stateChangeInfo["cellEdit"]["rowId"]] =
                new_patient_update;
        } else {
            let existing_patient_update_info = clone(
                copy_modified_patients_list[
                    stateChangeInfo["cellEdit"]["rowId"]
                ]
            );
            let data_field_key = stateChangeInfo["cellEdit"]["dataField"];

            if (type_str == "int") {
                existing_patient_update_info[data_field_key] = parseInt(
                    String(stateChangeInfo["cellEdit"]["newValue"])
                );
            } else if (type_str == "float") {
                existing_patient_update_info[data_field_key] = parseFloat(
                    String(stateChangeInfo["cellEdit"]["newValue"])
                );
            } else {
                existing_patient_update_info[data_field_key] = String(
                    stateChangeInfo["cellEdit"]["newValue"]
                ).toLowerCase();
            }

            copy_modified_patients_list[stateChangeInfo["cellEdit"]["rowId"]] =
                existing_patient_update_info;
        }

        if (type_str == "int") {
            copy_matrix_filtered[patient_edited_index][
                stateChangeInfo["cellEdit"]["dataField"]
            ] = parseInt(String(stateChangeInfo["cellEdit"]["newValue"]));
        } else if (type_str == "float") {
            copy_matrix_filtered[patient_edited_index][
                stateChangeInfo["cellEdit"]["dataField"]
            ] = parseFloat(String(stateChangeInfo["cellEdit"]["newValue"]));
        } else {
            copy_matrix_filtered[patient_edited_index][
                stateChangeInfo["cellEdit"]["dataField"]
            ] = String(stateChangeInfo["cellEdit"]["newValue"]).toLowerCase();
        }
        ...
        let column_obj_to_modify_index = together_data_columns.findIndex(
            (column_element) =>
                column_element["dataField"] ==
                stateChangeInfo["cellEdit"]["dataField"]
        );
        let column_obj_to_modify = clone(
            together_data_columns[column_obj_to_modify_index]
        );
        ...
        let map_col_values = together_patient_gene_information.flatMap(
            (item) => item[column_obj_to_modify["dataField"]]
        );
        map_col_values = map_col_values
            .slice(0, patient_edited_index)
            .concat(map_col_values.slice(patient_edited_index + 1));
        let col_unique = [...new Set(map_col_values)];

        let copy_together_cols = clone(together_data_columns);

        if (
            col_unique.includes(stateChangeInfo["cellEdit"]["newValue"]) ==
            false
        ) {
            if (typeof col_unique[0] == "number") {
                let converted_val = 0;
                if (Number.isInteger(col_unique[0])) {
                    converted_val = parseInt(
                        String(stateChangeInfo["cellEdit"]["newValue"])
                    );
                } else {
                    converted_val = parseFloat(
                        String(stateChangeInfo["cellEdit"]["newValue"])
                    );
                }
        ...
        if (col_unique.length < 3) {
                ...
                filterRenderer: (onFilter, column) => {
                    return (
                        <ProductFilter
                            onFilter={onFilter}
                            column={column}
                            optionsInput={select_options_col}
                        />
                    );
                },
            };
            ...
            copy_together_cols[column_obj_to_modify_index] = {
                ...
                filterRenderer: (onFilter, column) => {
                    return <NumberFilter onFilter={onFilter} column={column} />;
                },
            };
        } else {
            copy_together_cols[column_obj_to_modify_index] = {
